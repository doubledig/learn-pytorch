...
class myyolo(nn.Module):
    def __init__(self, device, input_size=None, num_classes=20, trainable=False, conf_thresh=0.001, nms_thresh=0.5,
        ...
        self.scale_torch = torch.tensor([[[input_size[1], input_size[0], input_size[1], input_size[0]]]], device=device)
        # we use resnet18 as backbone
        self.backbone = resnet18(pretrained=True)
        # neck
        self.SPP = nn.Sequential(
            Conv.Conv(512, 256, k=1),
            Conv.SPP(),
            Conv.BottleneckCSP(256 * 4, 512, n=1, shortcut=False)
        )
        self.SAM = Conv.SAM(512)
        self.conv_set = Conv.BottleneckCSP(512, 512, n=3, shortcut=False)
        self.pred = nn.Conv2d(512, 1 + self.num_classes + 4, 1)

    # 该网络使用空间节省时间，减少遍历取得grid坐标的复杂度
    ...
    def set_grid(self, input_size):
        self.input_size = input_size
        self.grid_cell = self.create_grid(input_size)
        self.scale_torch = torch.tensor([[[input_size[1], input_size[0], input_size[1], input_size[0]]]],
                                        device=self.device)
    ...
    def nms(self, dets, scores):
        ...
        while order.size > 0:
            ...
            w = torch.maximum(1e-28, xx2 - xx1)
            h = torch.maximum(1e-28, yy2 - yy1)
            inter = w * h
    ...
    def postprocess(self, all_local, all_conf, exchange=True, im_shape=None):
        ...
        prob_pred = prob_pred[(prob_pred.shape[0], cls_inds)]
    ...
    def forward(self, x, target=None):
        ...
        if not self.trainable:
            ...
            conf_loss, cls_loss, txtytwth_loss, total_loss = loss.loss(pred_conf=conf_pred, pred_cls=cls_pred,
